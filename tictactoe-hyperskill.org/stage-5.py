...
def checkwin():
    # Horizontal checks
    if check(grid[0], grid[1], grid[2]) is True or check(grid[3], grid[4], grid[5]) is True or check(grid[6], grid[7], grid[8]) is True:
        print(f"{turn} wins")
        exit()
    # Vertical checks
    if check(grid[0], grid[3], grid[6]) is True or check(grid[1], grid[4], grid[7]) is True or check(grid[2], grid[5], grid[8]) is True:
        print(f"{turn} wins")
        exit()
    # Diagonal checks
    if check(grid[0], grid[4], grid[8]) is True or check(grid[2], grid[4], grid[6]) is True:
        print(f"{turn} wins")
        exit()
